...
const selectCountry = document.querySelector("select.select-country");
const selectedCountry = document.querySelector('input.selected-country');


function handleSelectCountry() {
  selectCountry.innerHTML = '';
  for (let i = 0; i < c.length; i++) {
    selectCountry.innerHTML += `<option ${(selectedCountry && selectedCountry.value == c[i].code) ? 'selected' : ''} value="${c[i].code}">${c[i].Desc_ar + ' - ' + c[i].Desc_en}</option>`
  }
};
